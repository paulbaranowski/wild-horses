...
1. **Shows the queue**, scoped to the current repo by default (`--all` for every repo, `--repo <name>` for one other), by pasting `crew queue list --present` stdout. The CLI groups Queued, Needs an Agent, and Available under `##` headings and prints In flight / In review as unnumbered prose.
