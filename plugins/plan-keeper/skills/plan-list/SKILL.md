...
**Multiple roots:** the listing **unions across every plan root** automatically. You never choose a root to read from. When more than one root is configured, a plan in a **non-default** root is prefixed `root/...` (e.g. `personal/2026-…-foo.md`). A plan in the default root has no prefix. A single-root install shows no prefix. If the user wants only one tree ("just my personal plans"), add `--root <name>`.
