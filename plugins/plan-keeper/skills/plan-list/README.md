...
3. **Presents the result** by pasting `list --sections in-progress,in-review,todo,backlog` stdout. Do not rebuild the rows. Reports when active plans exist with an off-list status, and offers to show them. If the current repo has no active plans at all, lists the other repos under `~/plans/` so the user can pick one.
