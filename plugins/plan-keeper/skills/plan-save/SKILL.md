...
The defaults are a floor, not an override: if the user pipes in a body that already declares `Status:`, `Created:`, or `Kind:` in its own frontmatter, those values are kept untouched. Save never adds an `Agent:` tag — a freshly saved plan has none, and only `plan-crew` writes one when it promotes the plan to the groundcrew queue (a body that hand-declares `Agent:` is still preserved). `Status: backlog` means the plan is fetched but not dispatched (confirm via `crew status <id>`) — promote via `/plan-update` (or `file-meta update --field Status=todo`) when the plan is ready for groundcrew to pick up. `Created` is an ISO-8601 UTC save-time stamp that gives `plan-do`'s newest-first listing precise _intra-day_ ordering (filenames carry only a `YYYY-MM-DD` date, so without it same-day plans fell back to slug-alphabetical). It's persisted in frontmatter so status mutations — which rewrite the file and reset its OS timestamps — never disturb the order. `Kind` (omitted entirely if you don't pass `--kind`) records the document type — see [Classifying the Kind](#classifying-the-kind) and [../../plan-kinds.md](../../plan-kinds.md).
