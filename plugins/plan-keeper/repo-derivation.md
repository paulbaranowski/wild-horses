...
- **Reads never need a root.** `plan-do`, `plan-list`, the queue, and ticket resolution **union across every root** automatically. When more than one root exists, a plan in a non-default root is labelled `root/...` so two same-named plans from different trees stay distinguishable. An unprefixed row is the default root. Never ask the user which root to read from; show them all. A user who wants to narrow can pass `--root <name>`.
