...
| `timed-out`                | `git pull` exceeded the timeout (slow/unreachable remote); the pull was killed, repo left untouched                                                                            | report with the `error` field; suggest checking the remote/network, or raise `UPDATE_GIT_REPOS_TIMEOUT` |
