...
| `timed-out`                | `git pull` exceeded the timeout (slow/unreachable remote); the whole git process group — `fetch`/`index-pack` included — was killed and its partial pack cleaned up, repo left untouched                  | report with the `error` field; suggest checking the remote/network, or raise `UPDATE_GIT_REPOS_TIMEOUT` |
