...
3. **Handles dirty repos** whose effective action is `ask` - a stash-pull-pop or skip, per-repo. The user can set a lasting default (or a per-repo override) with `set-action` so they stop being asked.
