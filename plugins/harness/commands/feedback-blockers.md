...
9. **Generate paired test tasks** — for each intervention with `createsNewCode: true`, generate a companion test task placed immediately after it in the task ordering. The test task must:
   ...
   Interventions with `createsNewCode: false` do not get a paired test task — the existing tests are the safety net and the task's own `agentValidations` are the verification. See step 7 above and `task-list-schema.md`'s "Paired test tasks (rule)" for the full decision table on when to set `createsNewCode: true` vs `false`.
