...
- **Pydantic ↔ Pydantic roundtrip through a TypedDict** — two Pydantic models communicating via an intermediate TypedDict, with `cast(SomeTypedDict, model.model_dump())` on the producer side and `from_X(cls, data: SomeTypedDict)` (using `data["key"]` access) on the consumer side. Markers: `cast\([A-Z]\w+, .*\.model_dump\(\)\)`; a `TypedDict` class whose fields mirror a Pydantic model 1:1; a `from_<payload>` classmethod that builds a Pydantic model from `data["field"]` reads. The `cast` is a typing lie pyright never verifies — keys-vs-fields agreement is asserted, not checked — and the agent has to mentally translate between two type representations at every boundary instead of following a single typed object through the codebase. The consumer's `from_X` classmethod often hides defensive checks for impossible inputs (`if "id" in payload and "detection_id" not in payload`) that encode contradictions of the function's own type signature. Fix: replace the triad with a direct `to_<consumer>(self) -> ConsumerModel` method on the producer using attribute access; delete the TypedDict and the `from_X` classmethod. EXCEPTIONS (NOT this anti-pattern): `model_dump(exclude_none=True)` for `**kwargs` expansion to a keyword-argument function (legitimate, because the destination signature really is positional-by-name); `to_serializable_dict()` for JSON serialization to a non-Python consumer (legitimate boundary); TypedDicts typing genuinely external JSON input (request bodies, file parsers) before any Pydantic validation (legitimate wire-level shape).
