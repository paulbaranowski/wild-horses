...
- **Class in wrong package layer** — a structured-type class (Pydantic `BaseModel`, TypeScript `interface`/`class`) lives in a package whose `__init__.py` / index file re-exports a coherent category (e.g. HTTP request/response types consumed by route handlers' `response_model=`), but the class itself is NOT in that re-export list AND every non-test importer is in a sibling layer (e.g. `services/`, `domain/`). The package-level re-export list is the unwritten invariant; the omission is the package itself signaling the class doesn't belong. Corroborating signals, each independently raising confidence: the docstring announces a primitive→structured-type upgrade history ("Replaces the raw tuple", "Previously a Dict", "Upgrades the raw <type>") — those classes usually landed here because "Pydantic class → schemas/" was the upgrader's default mental rule rather than a deliberate layer choice; the class has a field whose declared type is a service class (a live Python instance with methods, not data); the file contains a `Protocol` declaration (service-collaborator interfaces don't belong in schemas). Suppress: classes used by both routes AND services (legitimate shared shape that crosses the HTTP boundary); enums shared as cross-layer vocabulary; classes consumed by any `response_model=` parameter on a route decorator anywhere in the codebase. Fix: move the class next to its producer (e.g. `services/<feature>_result.py`); if the source file becomes empty, delete it; update all importers. The move often eliminates a forward-reference workaround at the same time — see "Circular-import workaround cluster".
- **Circular-import workaround cluster** — three constructs spread across two or more files that exist *only* to break an import cycle: (1) a `TYPE_CHECKING` import of `X` in module A, (2) a string forward-reference annotation `Optional["X"]` (or `: "X"`) on at least one field of a class `C` defined in module A, and (3) a `C.model_rebuild()` (or Pydantic v1 `update_forward_refs()`) call in module B, where module B imports `X` at top level. Each piece compiles and type-checks cleanly in isolation — pyright is happy with all three. The actual import-time data flow ("`C` can't construct correctly until module B is imported and runs the rebuild") is invisible without reading all three pieces together. The cluster usually exists because `C` is in the wrong package (see "Class in wrong package layer") and the developer reached for forward refs to paper over the cycle. Report as a single composite finding spanning all the pieces, not three independent findings. Fix, in preference order: (1) move `C` next to `X` — usually right when `C` is service-internal; (2) extract the dependency both modules share into a new leaf module, eliminating the cycle entirely; (3) replace the concrete `X` annotation with a `Protocol` capturing only what `C` actually uses. Suppress: genuinely recursive types (a `Tree` whose field references itself) — those use same-module forward refs with no cross-module `model_rebuild`. Suppress: Pydantic v1 codebases where `update_forward_refs()` is part of every model's initialization (pattern is endemic and the detector would fire everywhere).
