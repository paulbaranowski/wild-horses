#!/usr/bin/env bash
# unresolvedPrComments.sh — Fetch review data for pr-babysit.
#
...
#     per-comment fingerprint is NOT already listed in any prior pr-babysit
...
#     pr-babysit (or legacy cb-babysit) sentinel prefix. Used for dedupe.
...
main() {
  ...
  local threads_json
  threads_json="$(printf '%s' "$response" | jq --arg sentinel_prefix "$SENTINEL_PREFIX" --arg legacy_cb_sentinel_prefix "$LEGACY_CB_SENTINEL_PREFIX" --arg legacy_babysit_pr_sentinel_prefix "$LEGACY_BABYSIT_PR_SENTINEL_PREFIX" --argjson bots "$BOTS_JSON" '
    def is_bot: ((.author.__typename // "") == "Bot") or ((.author.login // "") | IN($bots[]));
    def is_sentinel: ((.body // "") | contains($sentinel_prefix) or contains($legacy_cb_sentinel_prefix) or contains($legacy_babysit_pr_sentinel_prefix));
    [
  ...
  raw_issue_comments="$(printf '%s' "$response" | jq --arg sentinel_prefix "$SENTINEL_PREFIX" --arg legacy_cb_sentinel_prefix "$LEGACY_CB_SENTINEL_PREFIX" --arg legacy_babysit_pr_sentinel_prefix "$LEGACY_BABYSIT_PR_SENTINEL_PREFIX" --argjson bots "$BOTS_JSON" '
    def is_sentinel_body: ((.body // "") | contains($sentinel_prefix) or contains($legacy_cb_sentinel_prefix) or contains($legacy_babysit_pr_sentinel_prefix));
    def is_bot_author: ((.author.__typename // "") == "Bot") or ((.author.login // "") | IN($bots[]));
    [
  ...
  # in the fenced block at the end of a pr-babysit summary).
  ...
  # NOT already listed in any prior pr-babysit summary.
