...
Italicized. Substitute `<viewer-login>` from `gh api user --jq .login`. Non-negotiable - collaborators must be able to tell at a glance the review is AI-assisted. Do **not** mention the review skill by name; it's a local skill the PR author can't see or use.
