...
- Mutating a shared array/object in place (`sort`, `reverse`, `splice`, direct field writes on a passed-in reference) when a caller may still hold that reference; prefer the immutable form (`toSorted`, `toReversed`, a copy). The failure_mode is a real aliasing bug: the caller observes a value it never changed.
- **When the diff is shared-library or package code** (published packages, or a shared `lib`/`shared` module consumed across the codebase): the public surface takes object arguments and returns objects rather than long positional lists; the public API type does not expose `any` or a bare `object`; the surface is exported through the package's index/entry point, not reached into via deep internal paths. Raise only concrete violations on the code in the diff.
- **When the diff touches background jobs, queues, or message handlers:** the handler is idempotent (re-running on redelivery does not double-apply); it fetches fresh state by ID inside the handler rather than trusting a possibly-stale enqueued payload; enqueued payloads are minimal and serializable (IDs, not live objects or class instances); a message is published atomically with the DB write it reflects (enqueued inside the same transaction / via an outbox), not fire-and-forget after commit; the handler throws only for retryable errors and returns a descriptive skip for non-retryable ones. Each is raisable only with a concrete failure_mode (duplicate side effect, lost message, acting on stale data).
...
- Prefer exercising the public contract (an endpoint, an exported function, a rendered component) with real local dependencies, faking only slow or external services, over unit tests bound to internal structure. Reserve unit tests for pure logic, hard-to-trigger error paths, concurrency, and behavior with many variations.
- A test that mocks the very unit under test, or asserts a mock was called rather than an observable side effect (returned value, DB row, published message, rendered output), verifies nothing - raise it. This is the AntiSlop "tests that exercise the framework" smell made concrete.
- One behavior per test; each test sets up its own data rather than depending on shared mutable state or ordering.
- For UI, prefer user-centric queries (by role, label, or visible text) over test-id or DOM-structure selectors, so the test survives refactors and reflects what a user perceives.
...
- Incomplete dead-code removal: when the diff deletes the last usage of a symbol (function, constant, type, component), the symbol itself and any imports left unused should go too, recursively. Flag now-orphaned definitions and dangling imports the diff left behind.
- Cryptic acronyms or abbreviations that obscure meaning (widely-known ones in camelCase such as `httpRequest` are fine), and module-scoped constants that are not `SCREAMING_SNAKE`. Only raise names that mislead or genuinely obscure, never aesthetic preference.
...
- **Read-modify-write races.** A read-then-check-then-write against a value other requests can change concurrently (balance, counter, status transition, inventory) can lose updates under real traffic. Fold the current-state condition into the `UPDATE ... WHERE` (or `findOneAndUpdate` filter), or use `SELECT FOR UPDATE` / a serializable transaction / optimistic concurrency (version column). The failure_mode is concrete: two interleaved requests both pass the check and one write is lost.
