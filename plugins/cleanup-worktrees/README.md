...
- A worktree carrying commits preserved nowhere durable is never removed in the first place - it is skipped as `unpushed`. The `branch-prune-skipped` warning is a belt-and-suspenders fallback _after_ an already-safe removal: if `git branch -D` cannot delete the orphaned branch (for example it is checked out in another worktree), the worktree is gone but the branch ref is kept and the warning is reported. So no path removes a worktree's unique local commits.
