...
| **[`churn`](skills/churn/)**                   | `/wild-pr:churn`          | Diagnoses why a PR keeps going through review rounds: clusters the findings by root cause, runs five parallel lens agents, and recommends keep patching, refactor, split, or relax requirements.                   |
...
`churn` is backed by `scripts/pr_churn_cli.py collect`. It reads a PR's commits, reviews, threads, and comments through `gh`, writes a `timeline.json` that tags each finding with its review round, and prints the churn metrics. It posts nothing.
