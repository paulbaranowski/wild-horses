...
| **[`churn`](skills/churn/)**                   | `/wild-pr:churn`          | Diagnoses why a PR keeps going through review rounds: clusters the findings by root cause, runs six parallel lens agents, and recommends keep patching, refactor, split, relax requirements, or move the change to the component that should own it. |
...
`babysit` is backed by a bundled CLI, `scripts/pr_babysit_cli.py`, with subcommands `review` (fetch unresolved threads, automated review bodies, and conversation comments as structured JSON), `failed-logs` (stream failing-check output), `commit-push` (explicit-staging commit + push, never `git add -A`), `reply` (post a threaded reply), and `comment` (post a top-level PR summary). A PreToolUse hook (`hooks/hooks.json` + `scripts/pr-cli-allow.sh`) auto-approves bounded invocations of that CLI and of `churn`'s CLI - it rejects flags, chaining, and substitution, so it isn't a blanket approval.

`churn` is backed by `scripts/pr_churn_cli.py`. `collect` reads a PR's commits, reviews, threads, and comments through `gh`, writes a `timeline.json` that tags each finding with its review round, and prints the churn metrics. `boundaries` reads the local checkout's agent docs, manifests, env templates, client base URLs, and API schemas, and writes `boundaries.json`. Neither subcommand posts anything, and `boundaries` makes no network call.
