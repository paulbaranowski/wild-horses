...
- The contract below, verbatim (substitute `<context_ref>` from the freshness preflight):
...
Required output per finding: `id` (letter-prefixed: `B1`, `C3`, `S1`, …), `severity`, `file:lines`, `title` (one sentence), `point` (one short paragraph), `failure_mode` (one sentence), and optional `suggested_fix` (schema in rubric §Admission).
