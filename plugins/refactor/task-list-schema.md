...
- `loop-protocol.md` — the Phase 4 options menu shared by `/refactor:feedback-blockers` and `/refactor:reasoning-gaps` delegates file production to `task-list-builder`, which writes files in this shape.
...
| `in-progress` | `drafted`     | `draft --id N --commit-msg ...`       | Writes the log into the task; writes the commit subject to a per-task staging file at `/tmp/refactor-stage-<hash>-<N>.json`. Does not touch git.                                                             |
