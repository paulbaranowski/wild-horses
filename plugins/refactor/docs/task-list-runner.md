...
| Test discipline   | Optional — author choice                                       | Mandatory paired `"Write tests for X"` task after every `createsNewCode: true` task (new code or behavior change) |
...
| Typical scale     | A handful of well-scoped tasks                                 | 10–50 uniform tasks (the typical `/refactor:reasoning-gaps` or `/refactor:feedback-blockers` output)                |
